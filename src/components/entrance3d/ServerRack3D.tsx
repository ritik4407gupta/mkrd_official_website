      ...
      {[-0.43, 0.43].map((x, xi) =>
        [-0.33, 0.33].map((z, zi) => (
          <mesh key={`pillar-${xi}-${zi}`} position={[x, 1.25, z]}>
            ...
            <meshStandardMaterial color={hovered ? '#8B7DFF' : '#1e3a8a'} metalness={0.9} roughness={0.1} />
          </mesh>
        ))
      )}
      ...
      >
        ...
        >
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-900/90 border border-brand-500/60 shadow-[0_0_20px_rgba(124,113,255,0.4)] backdrop-blur-md text-white font-mono text-[11px] whitespace-nowrap">
            ...
            <span className="font-bold text-brand-300">CLOUD NODE 01</span>
            <span className="text-[10px] text-slate-400">|</span>
            <span className="text-[10px] text-emerald-400 font-semibold">100Gbps FIBER</span>
          </div>

          {hovered && (
            <div className="p-3 rounded-xl bg-slate-950/95 border border-brand-500/40 shadow-2xl backdrop-blur-md text-left text-xs max-w-[240px] text-slate-200 animate-in fade-in zoom-in-95 duration-200">
              <div className="font-bold text-brand-400 text-xs mb-1 font-display">MKRD Enterprise Node</div>
